@echo off
title SkySorterSuite Setup and Run
...
echo Creating SkySorter Suite local shortcut...
powershell -Command "$WshShell = New-Object -ComObject WScript.Shell; $Shortcut = $WshShell.CreateShortcut(\"%CD%\\SkySorter Suite.lnk\"); $Shortcut.TargetPath = 'pythonw.exe'; $Shortcut.Arguments = '\"%CD%\\_internal\\UI.pyw\"'; $Shortcut.IconLocation = '%CD%\\_internal\\_gfx\\UI.ico'; $Shortcut.WorkingDirectory = '%CD%'; $Shortcut.Save()"
